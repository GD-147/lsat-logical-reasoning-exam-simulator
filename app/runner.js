...
async function loadQuestionsForConfiguredSection(examId, cfg, section) {
  const sourceIds = section.sourceSections || [];

  if (!sourceIds.length) {
    return loadQuestionsForSection(examId, section);
  }

  const allSets = [];

  for (const sourceId of sourceIds) {
    const source = cfg.sections.find(s => s.id === sourceId);

    if (!source) {
      console.warn(`Missing source section: ${sourceId}`);
      continue;
    }

    const loaded = await loadQuestionsForSection(examId, source);
...
(async function () {
  ...
  // ========================================================
  // STANDARD LOGICAL REASONING MCQ RUNNER
  // ========================================================

  // Carica domande MCQ (supporta più examFiles)
  const examSets = await loadQuestionsForConfiguredSection(examId, cfg, section);
...
if (metaEl) metaEl.textContent = metaText;
  ...
  function finish() {
    ...
    qs("scoreLine").textContent =
      `Score: ${pct}% — ${correct} correct, ${incorrect} incorrect, ${mcqQs.length} total.`;
